...
const ItemGrids = ({item, user, index, setHomeIndex}) => {
    ...
    <div className="item-title"><Link to={`details/${item.item_slug}/pn/1`}>{item.item_name}</Link>
    <div className="item-price">{decode(item.item_price)}</div>
    <div className="item-old-price"><s>{decode(item.item_old_price)}</s></div>
    <div className="item-options">

    <div className="tooltip add-to-cart" onClick={()=>setHomeIndex(index)}><Link><FaShoppingCart /></Link><span className="tooltiptext">Add to cart</span></div>
